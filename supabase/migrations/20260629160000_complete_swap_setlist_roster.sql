-- Make swaps "complete" in the setlist roster (setlist visibility + publish
-- notifications), to match the calendar/Set Planner roster behavior.
--
-- Previously the swapped_out CTE only released the SINGLE position named on the
-- swap_requests row:
--
--   JOIN base_roster br
--     ON br.user_id = sr.requester_id
--    AND (normalize_position_token(br.position)      = normalize_position_token(sr.position)
--      OR normalize_position_token(br.position_slot) = normalize_position_token(sr.position))
--
-- So a volunteer who served two roles on a date (e.g. Graphics AND Hand-Held
-- Camera) and swapped one of them stayed on the roster via the other role -- they
-- could still confirm / were still notified for that date even though they had
-- handed the date off, and the person covering only partially took over.
--
-- A swap is a COMPLETE swap: the person who is out is out for ALL of their
-- positions on that service date, and the covering person takes over all of them.
-- Drop the position-match join condition so swapped_out releases every base_roster
-- row for the swapped-out user. This also makes fill-in covers fully release the
-- requester (which already matched the intended cover semantics). swapped_in is
-- unchanged: it adds the covering user at the membership level, which is enough for
-- both the boolean eligibility check and the notifiable-users list.
...
AS $$
  SELECT
    ...
    EXISTS (
      ...
      swapped_out AS (
        -- COMPLETE swap: the requester is out for ALL of their positions on the
        -- original_date (no position-match condition).
        ...
        UNION
        -- COMPLETE swap: the accepter is out for ALL of their positions on the
        -- swap_date (no position-match condition).
