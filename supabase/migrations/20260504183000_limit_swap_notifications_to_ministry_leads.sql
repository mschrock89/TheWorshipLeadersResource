...
begin
  ...
  if new.status in ('accepted', 'declined') and old.status = 'pending' then
    ...
    if supabase_url is not null and service_key is not null then
      ...
      -- If accepted, notify only the ministry lead(s) for this campus + ministry.
      if new.status = 'accepted' and swap_campus_id is not null then
        ...
        where (
            (normalized_swap_ministry_type = 'video' and ur.role = 'video_director')
            or (normalized_swap_ministry_type = 'production' and ur.role = 'production_manager')
            or (
