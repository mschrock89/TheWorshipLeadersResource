-- Prevent direct swaps on one team from removing a user from unrelated teams
-- when computing setlist confirmation eligibility.
--
-- swap_requests.team_id stores the requester's home team, so on the swap_date
-- side we cannot rely on sr.team_id matching the scheduled team. Instead, scope
-- swap-date roster changes to the matching position on the effective base roster.
