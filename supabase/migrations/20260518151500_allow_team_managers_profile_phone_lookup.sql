-- Let campus/team managers resolve campus roster contact details for group text.
-- Some managers, including campus_admin rows scoped by admin_campus_id, do not
-- necessarily have user_campuses rows themselves.
...
AS $$
  ...
  profile_scope AS (
    SELECT
      ...
      EXISTS (
        ...
        JOIN public.user_campuses profile_uc
          ON profile_uc.campus_id = ur.admin_campus_id
        JOIN viewer v ON true
        WHERE ur.user_id = v.viewer_id
          AND ur.role = 'campus_admin'::app_role
          AND ur.admin_campus_id IS NOT NULL
          AND profile_uc.user_id = p.id
      ) AS managed_campus,
      EXISTS (
        SELECT 1
        FROM public.user_ministry_campuses viewer_umc
        JOIN public.user_campuses profile_uc
          ON profile_uc.campus_id = viewer_umc.campus_id
        JOIN viewer v ON true
        WHERE viewer_umc.user_id = v.viewer_id
          AND profile_uc.user_id = p.id
      ) AS ministry_campus
  ...
  SELECT
    ...
    CASE
      ...
      WHEN v.is_team_manager AND (p.shares_campus OR p.managed_campus OR p.ministry_campus) THEN p.email
    ...
    CASE
      ...
      WHEN v.is_team_manager AND (p.shares_campus OR p.managed_campus OR p.ministry_campus) THEN p.phone
    ...
    CASE
      ...
      WHEN v.is_team_manager AND (p.shares_campus OR p.managed_campus OR p.ministry_campus) THEN p.birthday
    ...
    CASE
      ...
      WHEN v.is_team_manager AND (p.shares_campus OR p.managed_campus OR p.ministry_campus) THEN p.anniversary
  ...
  WHERE
    ...
    AND (
      ...
      OR v.is_pastor
      OR p.shares_campus
      OR (v.is_team_manager AND (p.managed_campus OR p.ministry_campus))
    );
$$;
