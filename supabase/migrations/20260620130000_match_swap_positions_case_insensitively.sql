-- Swap roster detection compared swap_requests.position to the base roster
-- position with an exact, case-sensitive string match. In practice the two
-- sides store the same role in different shapes:
--   * legacy / slot-sourced swaps store the machine value ("drums", "announcement")
--   * standardized team_members.position holds a display label ("Drums",
--     "Announcements", "Vocalist") with the machine value in position_slot
--     ("drums", "announcement", "vocalist_4")
--   * some swaps store a numbered display label ("Vocalist 4") whose slug is the
--     position_slot ("vocalist_4").
-- The exact match meant a swapped-in volunteer was never added to the effective
-- roster, so they could not see the weekend setlist in "My Setlists" even though
-- the calendar showed them serving.
--
-- Fix: normalize both sides to a slug (lowercase, non-alphanumeric runs collapsed
-- to "_") and match swap positions against either the roster position or its
-- position_slot. The swapped_out branches now emit the actual base-roster position
-- so the downstream removal (so.position = br.position) stays consistent.

CREATE OR REPLACE FUNCTION public.normalize_position_token(p_value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
