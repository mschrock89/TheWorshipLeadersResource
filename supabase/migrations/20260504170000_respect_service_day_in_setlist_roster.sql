-- Respect day-specific assignments (primarily Video team roles) when resolving
-- roster visibility for published setlists. This keeps weekend-pair access for
-- shared setlists while preventing Saturday/Sunday assignments from bleeding
-- into each other.
...
AS $$
  SELECT
    ...
    EXISTS (
      ...
      base_roster AS (
        SELECT DISTINCT tm.user_id, tm.position, tm.ministry_types, tm.service_day
      ...
      effective_roster AS (
        SELECT br.user_id, br.position, br.ministry_types, br.service_day
        FROM base_roster br
        WHERE NOT EXISTS (
          SELECT 1
          FROM swapped_out so
        ...
        UNION
        SELECT si.uid AS user_id, si.position, NULL::text[] AS ministry_types, NULL::text AS service_day
        FROM swapped_in si
