-- Shrink get_songs_with_stats payload: keep all-time aggregates, but only
-- return individual usage rows for the last 24 months + upcoming dates.
-- Set Planner availability uses a 12-month window; Songs UI can derive
-- campus/ministry filters from the recent usages without shipping years of history.
...
AS $function$
  ...
  usage_window AS (
    SELECT ((SELECT d FROM today) - INTERVAL '24 months')::date AS start_date
  ),
  song_usages AS (
    SELECT
      ps.song_id,
    ...
    JOIN public.service_plans sp ON ps.plan_id = sp.id
  ),
  song_stats AS (
    SELECT
      su.song_id,
      ...
      COALESCE(
        jsonb_agg(
          ...
          ORDER BY su.plan_date DESC
        ) FILTER (
          WHERE su.plan_date >= (SELECT start_date FROM usage_window)
        ),
        '[]'::jsonb
      ) AS usages
    FROM song_usages su
