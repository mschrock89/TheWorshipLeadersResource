-- Generalize "session set" roster handling so Student Camp (Morning/Evening) works
-- exactly like Kids Camp (Morning/Afternoon): the sets may be linked to a custom service
-- for service-flow/date scoping, but their roster, confirmations, and publish-notification
-- recipients all come from the Team Builder schedule under the BASE ministry type.
--
-- Two RPCs encode the Kids-Camp-specific normalization and must be widened:
--   * is_user_on_setlist_roster   (confirm eligibility / playlist visibility guard)
--   * get_setlist_notifiable_user_ids (push-notification recipients on publish)
--
-- We normalize any "<base>_morning|_afternoon|_evening" session ministry to its base,
-- and route every session-set variant through the Team Builder roster branch instead of
-- custom_service_assignments. Adding another multi-session service later only requires
-- extending the variant lists below (and the matching client constants).
