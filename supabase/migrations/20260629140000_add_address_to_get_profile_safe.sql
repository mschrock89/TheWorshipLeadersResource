-- Recreate get_profile_safe to also return the profile's home address. The
-- address column already exists on profiles (see add_profile_address migration)
-- and the profile editor needs the RPC to surface it. Address is treated as
-- sensitive contact info and masked with the same consent rules as phone.
