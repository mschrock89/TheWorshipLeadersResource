-- Custom-service setlists must use only custom_service_assignments, never the
-- Team Builder rotation (e.g. T3 on an HS Worship Prayer & Worship Night).
-- Also accept assignments stored on the service's original service_date when
-- the published set's plan_date doesn't have its own assignment rows.
--
-- Based on 20260701130500_make_setlist_roster_rpcs_campus_null_safe.sql.
...
AS $$
  SELECT
    ...
    EXISTS (
      ...
      JOIN custom_services cs
        ON cs.id = ds.custom_service_id
      WHERE ds.id = p_draft_set_id
        ...
        AND (
          csa.assignment_date = ds.plan_date
          OR (
            NOT COALESCE(cs.repeats_weekly, false)
            AND csa.assignment_date = cs.service_date
          )
        )
    )
    OR (
      NOT EXISTS (
        SELECT 1
        FROM draft_sets gated
        WHERE gated.id = p_draft_set_id
          AND gated.custom_service_id IS NOT NULL
          AND gated.ministry_type NOT IN (
            'kids_camp', 'kids_camp_morning', 'kids_camp_afternoon',
            'student_camp', 'student_camp_morning', 'student_camp_evening'
          )
      )
...
AS $$
  ...
  UNION
  SELECT user_id FROM team_builder_users
