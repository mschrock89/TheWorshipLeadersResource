-- Kids Camp Morning/Afternoon setlists may be linked to a Kids Camp custom service
-- for service-flow/date scoping, but their roster still comes from Team Builder.
-- Keep playlist visibility, confirmations, and publish notification guards aligned
-- with the Team Builder roster shown in Set Builder.
...
AS $$
  SELECT
    ...
    EXISTS (
      ...
      kids_camp_set_ministries AS (
        SELECT unnest(ARRAY['kids_camp','kids_camp_morning','kids_camp_afternoon']) AS value
      ),
      ...
      ds AS (
        SELECT
          ...
          ministry_type,
          CASE
            WHEN ministry_type IN (SELECT value FROM kids_camp_set_ministries) THEN 'kids_camp'
            ELSE ministry_type
