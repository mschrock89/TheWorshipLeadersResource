-- Add gender targeting to calendar events. When target_genders is set (e.g. a
-- "Man Hang" men-only event), push notifications only go to users whose
-- profile gender matches, in addition to the existing campus/ministry scoping.

alter table public.events
  add column if not exists target_genders text[];

alter table public.events
  drop constraint if exists events_target_genders_check;

alter table public.events
  add constraint events_target_genders_check
  check (target_genders is null or target_genders <@ array['male', 'female']::text[]);

comment on column public.events.target_genders is
  'Optional gender targeting for the event (male/female). Null or empty means all genders.';

-- ---------------------------------------------------------------------------
-- notify_new_event — recreate with gender filtering layered on top of the
-- existing campus/ministry recipient scoping (last defined in
-- 20260704120000_scope_push_triggers_by_resource_app.sql).
-- ---------------------------------------------------------------------------
  ...
  begin
    perform net.http_post(
      ...
      body := jsonb_build_object(
        'title', 'New Event',
        'message', event_title || ' • ' || campus_label || ' • ' || ministry_label || gender_label || ' on ' || to_char(event_date, 'Mon DD, YYYY') || time_label,
        'url', '/calendar',
