-- Keep song stats aligned with plan history source-of-truth:
-- PCO service_plans only count through the cutoff date, and newer usage
-- should come from published draft_sets in the app.
...
AS $function$
  ...
  song_usages AS (
    ...
    WHERE sp.plan_date <= DATE '2026-01-17'
  ),
  song_stats AS (
    SELECT
      su.song_id,
