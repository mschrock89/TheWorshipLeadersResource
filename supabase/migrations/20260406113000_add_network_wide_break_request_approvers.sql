create or replace function public.is_network_wide_rotation_period(_rotation_period_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    select 1
    from public.rotation_periods
    where id = _rotation_period_id
...
as $$
  select
    ...
    or (
      ...
      and auth.uid() in (
        'dd1c6bc4-c527-4fa0-8ca1-8ed50a2674f9'::uuid,
        '22c10f05-955a-498c-b18f-2ac570868b35'::uuid
      )
    )
    or (
...
set search_path = public
as $$
  select public.can_view_break_request_row(_request_user_id, _rotation_period_id)
$$;

drop policy if exists "Leadership can view break requests" on public.break_requests;
drop policy if exists "Leadership can update break requests" on public.break_requests;
...
using (
  auth.uid() = user_id
  or public.can_view_break_request_row(user_id, rotation_period_id)
);

create policy "Leadership can update break requests"
on public.break_requests
for update
using (public.can_review_break_request_row(user_id, rotation_period_id));
