-- Allow manual Production/Video schedule pushes to resolve recipients from the
-- same authoritative roster RPC as the Calendar, scoped to a single scheduled team.

DROP FUNCTION IF EXISTS public.get_roster_notifiable_user_ids(date, uuid, text);
