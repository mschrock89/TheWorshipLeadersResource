-- Serving / setlist / video roster pushes must only notify people on the
-- active rotation for that campus and service date.
--
-- Previously, if a campus had no rotation_periods (Network Wide), 
-- get_roster_notifiable_assignments treated an empty rot.ids array as
-- "include every team_members row." Combined with shared team_schedule rows
-- expanding to every campus, stale T1 and future T3 members (e.g. Mark Fraser,
-- Eli Rhodes) received Saturday "You're Serving Today" pushes even though they
-- were not on Calendar's current T2 roster.
--
-- Empty rotation set now means no recipients. Members and date overrides must
-- belong to the resolved period(s). get_roster_notifiable_user_ids wraps this
-- function, so setlist and video reminders pick up the same rule.
