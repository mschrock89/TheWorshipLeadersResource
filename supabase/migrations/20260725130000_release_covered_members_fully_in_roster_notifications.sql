-- Follow-up to 20260725120000: normalized position matching alone still left
-- covered members on the serving-reminder push list, because
-- get_roster_notifiable_user_ids removed only the single position named on the
-- swap request. The Calendar (useTeamRosterForDate) treats an accepted cover or
-- direct swap as a COMPLETE release: on the original date the requester is out
-- for ALL of their positions on that team (and on the swap_date side the
-- accepter is out for all of theirs), and the covering member takes over. A
-- member with a second (often stale) roster row on the same team — e.g. covered
-- on "AG 1" but still holding a "Vocalist" row — kept receiving "You're
-- Serving" pushes.
--
-- The Calendar and the setlist roster functions also match swaps against the
-- whole weekend pair (a Saturday cover releases the member for Sunday too);
-- this function compared original_date/swap_date to the single target date
-- only, so weekend covers recorded on the other service day were ignored.
--
-- Fix: mirror the Calendar semantics — release the requester (or swap_date
-- accepter) user-wide for the scheduled team, match swaps against the weekend
-- pair, and add the covering member whenever the released member holds any
-- roster row for the date.
...
AS $$
  WITH
    ...
    -- Swaps recorded on either day of the weekend pair release the member for
    -- both services, matching the Calendar and setlist roster behavior.
    swap_match_dates AS (
    ...
    -- An accepted cover/swap fully releases the member: the requester on the
    -- original date, and the accepter on the swap_date side of a direct swap.
    swapped_out AS (
      SELECT sr.requester_id AS uid
      FROM swap_requests sr
      WHERE sr.original_date IN (
          SELECT service_date FROM swap_match_dates WHERE service_date IS NOT NULL
        )
