-- Make the setlist roster + notification RPCs campus-NULL safe so Network Wide
-- Student Camp saved sets (draft_sets.campus_id IS NULL) resolve a roster.
--
-- Only ONE thing changes vs. 20260629160000: the rotation_periods (`rot`) CTE used
-- strict `rp.campus_id = ds.campus_id`, which returns nothing when the set is
-- network-wide. For a network-wide set (ds.campus_id IS NULL) we now consider
-- rotation periods across every campus. The team roster itself is still anchored to
-- the team_schedule rows that actually cover the date (eligible_schedule_rows already
-- matches network-wide team_schedule via `ts.campus_id IS NULL`), so this only widens
-- rotation-period matching for network-wide sets and leaves per-campus sets unchanged.
  ...
  SELECT
    ...
    EXISTS (
      ...
      rot AS (
        ...
        CROSS JOIN ds
        WHERE (rp.campus_id = ds.campus_id OR ds.campus_id IS NULL)
