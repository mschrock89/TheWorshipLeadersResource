-- Fix direct-swap roster detection on the swap_date side.
-- swap_requests.team_id stores the requester's home team, so for the swap_date
-- branch we must key off the accepted user's scheduled-team membership instead
-- of requiring swap_requests.team_id to match the team scheduled that weekend.
