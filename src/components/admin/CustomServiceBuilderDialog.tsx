...
/** Existing Team Builder rosters for the rotation period covering the date, usable as a starting lineup. */
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: members, error: membersError } = await supabase
        .from("team_members")
        .select("team_id, user_id, member_name, position, position_slot")
        .in("rotation_period_id", periodIds)
      ...
      for (const team of teams) {
        const teamMembers = (members || [])
          .filter((member) => member.team_id === team.id && member.user_id)
          .map((member) => {
        ...
        if (teamMembers.length > 0) {
          rosters.push({ teamId: team.id, teamName: team.name, members: teamMembers });
        }
      }
      return rosters;
    },
...
export function CustomServiceBuilderDialog({
  ...
  const copyTeamRoster = (teamId: string) => {
    const team = copyableRosters.find((roster) => roster.teamId === teamId);
    if (!team) return;
    setRoster((prev) => {
      const next = [...prev];
