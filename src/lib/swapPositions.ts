// Keep in sync with public.canonical_swap_position and
// supabase/functions/_shared/swapPositions.ts.
...
export function canonicalSwapPosition(position: string): string {
  ...
  const electricVariants = new Set(ELECTRIC_POSITION_VARIANTS.map(normalizeSwapPositionToken));
  if (
  ...
  const acousticVariants = new Set(ACOUSTIC_POSITION_VARIANTS.map(normalizeSwapPositionToken));
  if (
